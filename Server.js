...
const followersFile = path.join(dataDir, 'followers.json');
const viewerCountFile = path.join(dataDir, 'viewerCount.json');
const messagesFile = path.join(dataDir, 'messages.json');
const slideshowDataFile = path.join(dataDir, 'slideshowData.json');
...
const safeLoad = (filePath, fallback) => {
    try {
        return fs.existsSync(filePath) ? JSON.parse(fs.readFileSync(filePath, 'utf8')) || fallback : fallback;
    } catch (err) {
...
const saveSlideshowData = () => saveToFile(slideshowDataFile, {
    advertMessages,
    noticeMessages,
    storedDatas: { advert: storedAdvertData, notice: storedNoticeData }
});
...
app.get('/get-stored-data', async (req, res) => {
  try {
    ...
    res.json({ storedBusinessData: businessResult.rows, storedMedicalData: medicalResult.rows });
  } catch (error) {
...
app.post('/register-business', async (req, res) => {
  const { name, category, industryOrService, licenseNumber, location, contact_info, city, type } = req.body;
  try {
    const existing = await pool.query('SELECT id FROM businesses WHERE LOWER(name) = LOWER($1) AND LOWER(city) = LOWER($2)', [name, city]);
    if (existing.rows.length > 0) return res.status(409).json({ message: 'This business is already registered.' });
    await pool.query(
      `INSERT INTO businesses (name, category, industry_or_service, license_number, location, contact_info, city, type)
...
app.post('/register-medical', async (req, res) => {
  const { name, category, industryOrService, licenseNumber, location, contact_info, city, type } = req.body;
  try {
    const existing = await pool.query('SELECT id FROM medical WHERE LOWER(name) = LOWER($1) AND LOWER(city) = LOWER($2)', [name, city]);
    if (existing.rows.length > 0) return res.status(409).json({ message: 'This medical service is already registered.' });
    await pool.query(
      `INSERT INTO medical (name, category, industry_or_service, license_number, location, contact_info, city, type)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
...
io.on('connection', (socket) => {
    ...
    socket.on('sendReply', ({ replyText, messageId, timestamp }) => {
        ...
        if (parent) {
            const reply = { replyText, sender: socket.username, timestamp: timestamp || Date.now() };
